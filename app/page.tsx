...
export default function HomePage() {
  ...
  return (
    <div className="min-h-screen bg-[#F0FFF4] flex flex-col items-center justify-center p-4 text-black font-sans relative">
      ...
      <motion.div animate={{ y: [0, -20, 0] }} transition={{ duration: 2, repeat: Infinity, ease: "easeInOut" }} className="text-6xl mb-4">🧺</motion.div>
      ...
      <div className="w-full max-w-sm bg-white p-6 rounded-[40px] shadow-2xl space-y-4 border border-green-50/50 text-center">
        <input value={name} onChange={e => setName(e.target.value)} placeholder={t.name} className="w-full p-4 rounded-[22px] bg-gray-50 border-none outline-none font-bold focus:ring-2 ring-green-300 transition-all uppercase text-sm" />
          ...
          {mainMode === 'solo' && (
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden space-y-3">
              ...
              <div className="flex gap-1 bg-gray-50 p-1 rounded-xl">
                <button onClick={() => setSoloMode('classic')} className={`flex-1 py-2 text-[9px] font-black rounded-lg transition-all ${soloMode === 'classic' ? 'bg-black text-white' : 'opacity-40'}`}>{t.classic}</button>
                <button onClick={() => setSoloMode('ai_guesses')} className={`flex-1 py-2 text-[9px] font-black rounded-lg transition-all ${soloMode === 'ai_guesses' ? 'bg-black text-white' : 'opacity-40'}`}>{t.aiGuesses}</button>
              </div>
              <p className="text-[9px] opacity-40 font-bold px-2 text-left">
                {soloMode === 'classic' ? t.classicDesc : t.aiGuessesDesc}
              </p>

              {/* СЛОЖНОСТЬ — только в классическом */}
              {soloMode === 'classic' && (
                <div className="space-y-2">
                  <p className="text-[9px] font-black opacity-30 uppercase text-left">{t.difficulty}</p>
                  <div className="flex gap-1">
                    {(['easy', 'medium', 'hard'] as Difficulty[]).map(d => (
                      <button key={d} onClick={() => setDifficulty(d)} className={`flex-1 py-2 text-[9px] font-black rounded-lg transition-all border ${difficulty === d ? d === 'easy' ? 'bg-green-500 text-white border-green-500' : d === 'medium' ? 'bg-yellow-500 text-white border-yellow-500' : 'bg-red-500 text-white border-red-500' : 'opacity-30 border-gray-200'}`}>
                        {t[d]}
                      </button>
                    ))}
                  </div>
              ...
              {/* БЕЗ ЖИЗНЕЙ */}
              {soloMode === 'classic' && (
                <button onClick={() => setNoLives(!noLives)} className={`w-full py-2.5 rounded-[16px] text-[9px] font-black transition-all border-2 ${noLives ? 'bg-black text-white border-black' : 'bg-white text-black border-gray-200 opacity-50'}`}>
                  {noLives ? '∞' : '🍋'} {t.noLives}
                </button>
              )}
            </motion.div>
