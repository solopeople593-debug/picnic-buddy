  ...
  return (
    <div className="h-screen bg-[#F0FFF4] flex flex-col items-center justify-center p-6 text-black font-sans relative overflow-hidden">
        <div className="absolute top-6 right-6 flex gap-1 z-50">
            {['EN', 'UA', 'RU', 'LV'].map(l => (
                <button key={l} onClick={() => setLang(l as any)} className={`px-3 py-1.5 rounded-full text-[10px] font-black transition-all ${lang === l ? 'bg-black text-white' : 'bg-white border border-gray-100 shadow-sm'}`}>{l}</button>
            ))}
        </div>
