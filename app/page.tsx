...
export default function Home() {
  ...
  const t = {
    ...
    RU: { title: 'ПИКНИК БАДДИ', sub: 'Умная игра в поход с ИИ', name: 'Имя', namePl: 'напр. Соло Скайуокер', start: 'Начать', join: 'ВОЙТИ В ИГРУ', joinPl: 'КОД', solo: 'СОЛО', multi: 'МУЛЬТИ' }
  }[lang]
  ...
  const handleCreate = () => {
    ...
    // Соло идет сразу в игру, Мульти — в настройки
    router.push(isSolo ? `/game/${newCode}?mode=solo` : `/game/${newCode}/setup?mode=${mode}`)
  }
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-[#F0FDF4] p-6 font-sans text-black relative overflow-hidden">
      ...
      {/* Кнопка смены языка */}
      <button onClick={() => setLang(lang === 'EN' ? 'RU' : 'EN')} className="absolute top-6 right-6 bg-white px-3 py-1 rounded-full shadow-sm text-[10px] font-black border border-green-100">
        {lang === 'EN' ? '🇬🇧 EN' : '🇷🇺 RU'}
      </button>
      ...
      <div className="w-full max-w-sm space-y-8 relative z-10">
        ...
        <div className="bg-white p-8 rounded-[32px] shadow-2xl shadow-green-100/50 border border-green-50 space-y-6">
          ...
          <div className="bg-gray-100 p-1 rounded-2xl flex relative h-12">
            ...
            <button onClick={() => setIsSolo(true)} className="flex-1 text-[10px] font-black z-10">{t.solo}</button>
            <button onClick={() => setIsSolo(false)} className="flex-1 text-[10px] font-black z-10">{t.multi}</button>
          </div>
            ...
            {!isSolo && (
              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="space-y-2 overflow-hidden">
                {['ai_host', 'help', 'manual'].map(m => (
                  <button key={m} onClick={() => setMultiMode(m)} className={`w-full p-3 rounded-xl border text-left transition-all ${multiMode === m ? 'bg-green-50 border-green-500' : 'bg-white border-gray-100'}`}>
          ...
          <button onClick={handleCreate} className="w-full bg-[#22C55E] text-white font-black py-5 rounded-2xl shadow-lg shadow-green-200 active:scale-95 transition-all uppercase text-sm">{t.start}</button>

          <div className="flex items-center py-2 text-gray-200"><div className="flex-grow border-t"></div><span className="mx-4 text-[10px] font-black">{t.join}</span><div className="flex-grow border-t"></div></div>
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => router.push(`/game/${code}`)} className="bg-black text-white px-6 rounded-2xl font-black text-xs hover:bg-gray-800 transition-all">OK</button>
          </div>
        </div>

        <div className="text-center space-y-4">
          <p className="text-[10px] text-gray-300 font-bold uppercase tracking-[0.2em]">© 2026</p>
          <div className="inline-block px-4 py-1.5 bg-green-100 rounded-full"><span className="text-green-700 text-[10px] font-black tracking-widest uppercase">MADE BY SOLO</span></div>
        </div>
      </div>
    </div>
  )
